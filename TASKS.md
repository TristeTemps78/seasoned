...
| 7.16 | **Un `<p>` joue un titre dans `TrajectorySection`** | 🟢 libre | `TrajectorySection.tsx:119` — `<p className="text-sm font-medium">{t('entry.title')}</p>` titre l'encart du point d'entrée sans être un titre. ⚠️ **Question d'accessibilité, pas de typographie** : en faire un `h3` change l'arbre du document et la table des matières que lit un lecteur d'écran. Délibérément **pas** traité dans un lot sur les tailles — et donc invisible à `no-adhoc-typography`, qui ne regarde que `h1`–`h3` |
