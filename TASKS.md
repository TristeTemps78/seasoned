...
|---|---|
| 🔴 **Le chemin connecté, vu depuis un compte** | **La priorité, et le lot 10 vient de dire pourquoi.** La base porte **1 compte et 0 profil** : tout le social a été bâti sans qu'une seule ligne y passe. C'est exactement ce qui a laissé 10.0 invisible pendant trois sessions — un 400 y ressemblait à un démarrage à froid. Demande deux vraies sessions (lien magique, 2/h) |
| **9.3 — l'animation de bascule** | Le seul morceau des faces qui manque. Exige un champ de journal (la dernière face **annoncée**) ; le format est additif, donc gratuit à poser |
...
| **Nommer les trois faces** | « Va au bout », « Coupe net », « Revient toujours » décrivent le comportement — ils ne baptisent rien. **Choix de Tristan** |

✅ **Livré le 2026-08-10 (lot 10)** : les followers, « qui d'autre l'a vue » sur la fiche
série, Discover trié par ce qui se lit, **les trois faces** (9.1, 9.2, 9.4) — et 🔴 **le fil
et les critiques, qui n'avaient jamais rien pu lire** (voir 10.0 : la relation vers
`profiles` n'était déclarée nulle part, PostgREST répondait 400, et `#rows()` le rendait
invisible).
...
## 🤝 Lot 10 — le social réciproque, et les trois faces (2026-08-10) — ✅ **livré**

> **✅ Livré — @claude-opus — 2026-08-10.** 767 tests, build vert, `008`/`009`/`010`
> appliqués à la vraie base. **Non poussé** — c'est la décision de Tristan.
>
...
| # | Tâche | Statut | Note |
|---|---|---|---|
| 10.0 | 🔴 **Le fil et les critiques n'ont JAMAIS rien pu lire** | ✅ **2026-08-10 — @claude-opus** | **Trouvé en vérifiant le prérequis de 10.3, et c'est le vrai défaut de ce lot.** Mesuré par la porte du navigateur, avec la clé publique : `GET /rest/v1/reviews?select=subject,profiles!inner(handle,user_id)` → **400 `PGRST200`**, idem pour `activity`. Ce sont les **deux seules lectures sociales du produit** — fiche série, page de profil, fil de `/amis` — et elles rendaient `[]` **depuis toujours, pour tout le monde**. Cause : `reviews.user_id`, `activity.user_id` et `lists.user_id` référencent `auth.users`, jamais `public.profiles` ; le lien vers le profil était **vrai dans la tête de tout le monde et déclaré nulle part**, et PostgREST ne devine pas. ⚠️ **Invisible pour trois raisons qui s'additionnent** : (a) `#rows()` promet de ne jamais lever, donc un 400 y devient `[]` ; (b) les écrans se taisent quand la liste est vide — *mieux vaut se taire que compter zéro* ; (c) la base ne contient **aucun profil**, donc aucune absence ne pouvait surprendre. **L'écran d'un défaut était identique à celui d'un démarrage à froid.** 🔴 **Septième « une fonctionnalité écrite n'est pas une fonctionnalité qui marche »**, et cette fois la preuve manquante n'était ni un test ni un typage : **aucune requête PostgREST n'avait jamais été envoyée à la vraie base**. Les tests doublent `fetch` — ils vérifient l'URL qu'on construit, jamais qu'elle répond. `009_relations.sql` déclare les trois clés étrangères ; **revérifié après coup par la même porte** : 200 sur les trois lectures, et la **forme** prouvée avec de vraies lignes (`reviews:[{count}]`, `profiles:{handle,user_id}`), retirées ensuite |
| 10.1 | **Les followers** | ✅ **2026-08-10 — @claude-opus** | `008_followers.sql` : `follows_me()` en `security definer` (même forme que `can_see`, et pour la raison écrite dans `003_social.sql:172`) + une **seconde** politique `select` sur `profiles`, jamais une modification de l'existante — Postgres unit les politiques permissives en OU, donc les 11 scénarios de 8.13 ne bougent pas. 🔴 **Et la feature a rendu visible un trou** : `follows_insert_own` n'exigeait pas que celui qui suit ait réclamé un nom. Un compte sans profil suivait donc **sans jamais apparaître nulle part** — un observateur qu'on ne peut ni voir ni signaler, qui lit votre fil. `has_handle()` le refuse ; et le bouton « Suivre » de `PublicProfile`, qui s'affichait à ce compte-là, cède la place au geste qui débloque (règle du 2026-08-09 : *un bouton qui ne peut pas marcher ne s'affiche pas*). **7 scénarios rejoués contre la vraie base**, dans une transaction annulée — rien ne persiste, vérifié après coup. **Mutation** : sans la politique, « A voit le nom de B qui le suit » rend **0** |
| 10.2 | **« Qui a aimé cette série »** | ✅ **2026-08-10 — @claude-opus** | Zéro table neuve : `activity_select_visible` filtre déjà, donc un visiteur anonyme ne voit que les profils publics. ⚠️ **`liked` et `finished` seulement, et c'est une décision de spoiler** : ni l'un ni l'autre ne porte de numéro de saison, donc ils ne disent rien de l'intérieur de la série. Un `rated_season` en dirait — il exigerait `redactActivity` et la position du lecteur, la mécanique du fil, pour un encart qui n'en a pas besoin. **Mutation vérifiée.** ⚠️ **Aucun compteur, jamais** : RLS ne rend que les gens visibles par **ce** lecteur, donc un nombre varierait d'un visiteur à l'autre pour la même série |
| 10.3 | **Discover trié par ce qui se lit** | ✅ **2026-08-10 — @claude-opus** | `discoverable()` rendait les 24 derniers **inscrits**, par date : on ouvrait des profils vides. Elle rend les mêmes gens **ordonnés par ce qu'il y a à lire chez eux**, et le dit à côté du nom. 🔴 **Trier, jamais filtrer** : un filtre « a écrit quelque chose » aurait rendu `Discover` muet au démarrage à froid, c'est-à-dire au seul moment où il sert. ⚠️ Le vivier est **borné** (48 pour 24 affichés) : PostgREST ne sait pas trier par un compte imbriqué, et tout demander serait le coût non borné que ce produit refuse |
| 10.4 | **La face — les « équipes »** (9.1 · 9.2 · 9.4) | ✅ **2026-08-10 — @claude-opus** | **Le sujet de `face.ts` n'est pas l'arithmétique, c'est le choix de la matière** : elle ne regarde que `decision` et `completions` — des **événements**, qu'`importForeign` n'écrit pas — jamais les notes, jamais un fait portant `origin`. Les quatre pièges de Tristan tombent donc **sans une heuristique**. Vérifié sur un import tiers réel de 30 séries : aucune face. Fenêtre glissante (10 derniers faits, jamais les premiers), silence sous 5, et à égalité c'est la face **vers laquelle on vient de basculer** — jamais celle déclarée en premier dans le type (le défaut du `deviceId` de `sameJournal`). Des **noms** en base, pas des couleurs (leçon de `statusLabel`). `010_face.sql` : une colonne, **aucune politique nouvelle** — `profiles_update_own` couvre déjà le cas, vérifié en 4 scénarios. **3 mutations vérifiées.** ⚠️ **Les libellés des trois faces sont provisoires** : « Va au bout », « Coupe net », « Revient toujours » décrivent le comportement, ils ne baptisent rien. Les vrais noms sont un choix de Tristan |
| 10.4bis | **Le prélude : `origin` sur `decision` et `completions`** | ✅ **2026-08-10 — @claude-opus** | 🔴 Le champ manquait aux **deux seuls types dont la face a besoin**, alors qu'`asImported` les marquait déjà — elle marque par **parcours**. `parseDecision` et `parseCompletions` ne relisaient pas la marque : **écrite, puis effacée à la première sauvegarde**, avec tous les tests verts. Mot pour mot le défaut que 9.0 raconte avoir refermé, resté ouvert à côté. **Latent** aujourd'hui (`importForeign` n'écrit ni décision ni visionnage) — donc refermé pendant qu'il ne coûte rien, parce qu'un fait importé sans marque ne se distinguera **plus jamais** d'un fait vécu |

### ⛔ Ce qui reste ouvert après ce lot, et qu'il ne faut pas croire fait

| Ce qui n'a **pas** été vérifié | Pourquoi, et ce qu'il faudrait |
|---|---|
| **Le chemin connecté, vu depuis un compte** | Toujours ouvert, et le lot ne l'a pas fermé. La base de production contient **1 compte, 0 profil** : le social n'a jamais servi. Il faut **deux vraies sessions** — lien magique, 2 envois/heure, et le service intégré ne livre qu'aux membres de l'équipe |
| **Le cube de l'en-tête coloré par la face** | 🔴 **Non vu, et la mesure ne peut pas trancher.** La règle est bien dans la feuille servie et dans `@layer components` ; le cube de `/bilan` (48 px) mesure `top=1 left=0.22 right=0.22`, et un **clone** du cube d'en-tête, placé dans l'en-tête même, mesure `0.22`. Mais le nœud original y répond `1` — **et il répond `1` même après un `style.opacity = 0.5` inline**, donc c'est sa résolution de style qui est figée : le panneau navigateur n'étant pas affiché, la page ne composite pas. *La mesure ment ici ; seul un écran affiché tranchera.* |
| **9.3 — l'animation de bascule** | Hors périmètre, assumé. Elle exige un champ de journal (la dernière face **annoncée**) que 9.0 n'a pas posé. Le format est additif, donc elle ne coûtera pas plus cher plus tard — et elle n'a aucun sens tant que personne n'a vu la face fixe |
| **Les scénarios RLS ne sont pas rejouables** | Les 7 + 4 de ce lot ont tourné depuis un script du bloc-notes, comme les 11 de 8.13 — et comme eux, **ils ne survivent pas à la session**. Le procédé, lui, vaut d'être gardé : `begin` → semer → `set_config('request.jwt.claims', …)` → mesurer → `rollback`, rien ne persiste. Candidat à un `scripts/rls-scenarios.sql` le jour d'un prochain lot social |
