...
| 7.4 | **Les six écrans jamais revus** | 🔒 in-progress — @claude-opus — 2026-08-05 | ⚠️ Réservée pour `/hors-ligne` **seul** — les cinq autres restent à ouvrir. `/convertir`, `/regles`, `/mentions`, `/confidentialite`, `/hors-ligne`, `/compte/retour`. Les trois derniers sont ceux qu'on voit **quand quelque chose ne va pas** — donc ceux où la mise en page compte le plus |
...
| 7.13 | **Le grand chiffre du bilan est plus petit que le titre de la page** | 🔒 in-progress — @claude-opus — 2026-08-05 | `MyTally` porte `text-2xl` (24 px) et `.page-title` monte à 28 px : le **différenciateur du produit** — « au moins 537 heures » — passe visuellement après l'étiquette « Mon bilan ». Volontairement pas touché : ça se décide avec 7.6, qui traite du contenu de cet écran |
| 7.14 | **Le `/code-review` du lot 7 : neuf titres n'ont aucun cran, et la garde ne les voit pas** | 🔒 in-progress — @claude-opus — 2026-08-05 | 🔴 **Trouvé par relecture, et c'est le défaut que 7.5 annonçait avoir corrigé.** Il a réparé `Agenda`, `MyStats` et `Friends`, et **laissé sept** `h2` en `font-semibold` nu — plus deux `h3` que son motif (`<h[12]`) ne regarde même pas. `tests/no-adhoc-typography.test.ts` cherche une taille **en trop**, jamais une taille **absente** : la garde ne voit pas le défaut pour lequel elle a été écrite. Et `.section-title > :first-child` est **positionnel** — un élément inséré avant le `h2` lui retire tout, sans erreur ni test |
