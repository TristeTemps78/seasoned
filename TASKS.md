...
| 4.4b | ⚠️ **Câbler l'avertissement sur la page série** | 🟢 libre — **PRIORITAIRE** | 🔴 **`ordering.ts` n'est appelé par RIEN.** C'est exactement la forme d'échec du projet — *une fonctionnalité écrite n'est pas une fonctionnalité qui marche*, et `episodeMinutes` a été livré mort-né pour cette raison. Reste : un enveloppeur dans `lib/catalog.ts` (avec cache, +1 appel/série/24 h), le rendu, et **deux entrées de dictionnaire** (le test `no-hardcoded-strings` l'imposera). Formulation à respecter : **« voici la convention que suivent nos chiffres »**, pas « attention erreur » |
