...
| # | Tâche | Statut | Note |
|---|---|---|---|
| 5.0a | **La procédure : règles, motifs typés, procédure de retrait** | ✅ 2026-08-03 | `src/domain/moderation.ts` (pur) + **`/regles`** et `/fr/regles`, au sitemap et **en pied de page sur toutes les pages**. **655 → 682 tests**, 21 → **23 routes statiques**. Trois décisions figées : **on masque, on ne supprime jamais** (un retrait erroné doit être réparable, et le premier à se tromper sera nous) ; **tout retrait porte un motif typé** (une personne seule ne peut pas rédiger sur mesure — un motif typé se gabarise, donc l'auteur est toujours informé) ; **l'auteur peut contester** (`restore()` existe parce qu'un dispositif qui sait retirer mais pas rendre n'est pas de la modération). ✅ **La page ne peut pas mentir** : les motifs viennent de `REPORT_GROUNDS` et le délai de `REVIEW_DEADLINE_HOURS` — **mutation vérifiée : ajouter un motif au domaine sans le publier ne compile plus**. `triage()` empêche qu'un spoiler passe devant une menace. ⚖️ **Textes à faire relire — je ne suis pas une source juridique** |
