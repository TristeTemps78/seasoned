...
| **A13** | **Le produit suit-il les films ?** | ✅ **tranché par Tristan, 2026-08-03 : suivi complet** | **Contraire à ma recommandation** (« listes seulement »), actée — comme A1 et A7, mes objections deviennent le cahier des charges, pas un veto. ✅ **Bonne nouvelle trouvée en vérifiant : aucune migration de journal.** La clé est `<espace>:<id>` et `parseJournalKey` coupe au **premier** `:` — les séries gardent `tmdb:1396` **inchangé**, les films prennent un espace neuf. On ne migre pas ce qui marche déjà. 🔴 **Le vrai risque n'est pas les types, c'est que 8 modules supposent que toute entrée a des saisons** (`tally`, `remaining`, `trajectory`, `entry-point`, `catch-up`, `current-season`, `nudge`, `calendar`) : un film non filtré empoisonne **silencieusement** chaque agrégat. Détail et plan : `docs/IDEES-TRIEES.md` §1 |
...
| # | Tâche | Statut | Note |
|---|---|---|---|
| 5.10 | **A13 — le suivi des films** | 🟢 libre | ⚠️ **Le plus structurant du lot.** Aucune migration (voir A13), mais **8 modules à border** avant d'écrire la première entrée film, sinon les agrégats se faussent en silence. Bénéfice : un film a une durée, donc il **compte dans le bilan d'heures** |
| 5.11 | **Disponibilité multi-pays (au lieu de détecter le VPN)** | 🟢 libre | 🔴 **Le contre-sens le plus rentable du lot.** Détecter un VPN exige d'inspecter l'IP côté serveur — donc du **traçage**, ce qui casse l'argument qui rend A6 cohérent — et c'est **peu fiable** de toute façon. Or `/watch/providers` renvoie **tous les pays d'un coup**, dans l'appel qu'on fait déjà : « sur Netflix 🇬🇧, pas 🇫🇷 » est donc quasi gratuit, sans savoir où est qui. **On ne devine pas l'utilisateur, on le laisse choisir ses pays** |
...
| 5.18 | **Écran d'accueil fait de critiques** | ⛔ **contredit une décision fondatrice** | `ROADMAP.md` §0.1 : le corpus de textes **ne s'amorce pas**, donc une page doit valoir le détour **avec zéro critique**. Un accueil bâti sur des critiques est **vide le premier jour** — c'est exactement pourquoi les textes ont été mis en dernier et les données dérivées en premier. À reprendre comme « accueil qui se remplit », pas comme prérequis |
...
| 5.20 | **Scrobbling** | ⛔ pas maintenant, **moins cher plus tard** | Troisième demande. Rien n'a changé sur le coût *aujourd'hui* (première route serveur, jeton par utilisateur, `service_role`). Mais deux faits nouveaux : (1) **Serializd n'a aucun scrobbling** et c'est lui qui gagne cette niche — ce n'est donc pas la feature qui décide ; (2) **D17** : le natif rend le push obligatoire, donc un planificateur serveur existera de toute façon — le coût *marginal* d'un récepteur de webhooks baisse **après** cette infrastructure. Question d'**ordre**, pas de principe. D15 (Trakt) reste fermé |
