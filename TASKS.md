...
| 8.2 | **Le cœur** | 🟢 libre | Jumeau exact de `wanted` : booléen daté, série seulement. ⚠️ Corrige au passage `SocialClient.feed()` (`client.ts:214`) qui fait `row['kind'] as …` **sans valider** — un ancien client recevant `liked` rendrait une ligne muette. Règle 4 appliquée là où elle avait été sautée |
| 8.3 | **Les exceptions de progression** | 🟢 libre | Un seul enregistrement par épisode (`skipped \| watched`), donc exclusion mutuelle **vraie par construction**. ⚠️ Tombstone `mark:3:7` et **surtout pas** `episode:3:7`, déjà pris par la note d'épisode. 🔴 **Le piège du lot est dans `tally.ts:167`** : `vus = total − remaining`, donc brancher les marques naïvement ferait qu'un épisode **sauté** *monte* les heures vues — on compterait comme regardé ce qu'on a déclaré ne pas avoir regardé |
| 8.4 | **Les critiques : domaine et caviardage** | 🟢 libre | Le journal reste la **source de vérité** (règle 9 + Q12), la table porte la copie publiable. 🔴 Décisif : `001_journal.sql:96` interdit **par décision écrite** qu'un compte lise le journal d'un autre. Le caviardage se fait dans le **navigateur du lecteur** ; la borne de spoiler est **déclarée par l'auteur et typée** — « sans spoiler » ou « j'ai vu jusqu'à S{n} », confirmée, jamais devinée depuis sa position |
| 8.5 | **Réveiller le code mort du social** | 🟢 libre | `setVisibility()` et `unfollow()` n'ont **aucun appelant**, et `Friends.tsx:95` code `'followers'` en dur — donc **aucune critique ne serait lisible par personne**. Livre un test de conformité : *toute méthode publique de `SocialClient` a un appelant sous `app/`*. **Quatrième chose morte-née de ce dépôt** (`ordering.ts`, `episodeMinutes`, et ces deux-là) : cette fois la vérification devient exécutable |
| 8.6 | **La table `reviews` + la publication** | 🟢 libre | Clé naturelle `(user_id, subject, target)` : republier remplace, jamais ne duplique. `hidden_at` rend **exécutable** la promesse « on masque, on ne supprime jamais » de `/regles`. `lang` obligatoire — irrattrapable après coup (A9). Neuf scénarios rejoués **contre la vraie base** en transaction annulée : deux impasses SQL ont déjà été trouvées ainsi, en jouant ce qui était vert à la lecture |
| 8.7 | **Écrire** (UI) | 🟢 libre | ⛔ Publier ET lire conditionnés à `legalIsComplete()`. **Moitié levé le 2026-08-06** : `LEGAL_CONTACT_EMAIL` est renseigné localement et passe `looksLikeEmail` ; **`LEGAL_PUBLISHER_NAME` manque encore** (`lib/legal.ts:108` exige les deux). ⏳ Action de Tristan — personne ne peut l'inventer, c'est l'identité de l'éditeur, et il faut la poser **aussi chez Vercel** puisque `legal.ts` refuse que ces valeurs vivent dans le dépôt. Écrire dans son journal reste possible sans |
